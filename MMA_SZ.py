...
dic2017 = workdaydic(start = '2017/03/22', end = '2017/12/15')
LL = dic2017.values()
...
def processdata():
    ...
    for i in LL:
        try:
            ...
            format = lambda x: str(x)[-7:-1]+'.SZ' 
            table1['Symbol'] = table1.iloc[:,1].map(format)
            table1.to_csv('MMA_SZ%s%s%s.csv' % (i['Year'],i['Month'],i['Day']))
            driver.quit()            
            LL.remove(i)
        except:
            pass
